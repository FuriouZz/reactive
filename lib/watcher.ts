...
const items: [InternalObservable, string | symbol][] = [];
let listening = false;

/**
 * @internal
 */
export function registerWatch(p: InternalObservable, key: string | symbol) {
  if (!listening) return;
...
export function getWatchKeys<T>(
  cb: () => T,
  caller?: unknown
): [value: T, keys: [InternalObservable, string | symbol][]] {
  items.length = 0;
  listening = true;
